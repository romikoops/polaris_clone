...
tenant_data.each do |ten|
  ...
  load_types = [
   ...
   vehicle_types.each do |vt|
     mot = vt.split('_')[0]
     nvt = Vehicle.create(name:vt, mode_of_transport: mot)
     tenant.tenant_vehicles.create(name:vt, mode_of_transport: mot, vehicle_id: nvt.id)
     load_types.each do |lt|
       cargo_types.each do |ct|
         nvt.transport_categories.create(mode_of_transport: mot, cargo_class: lt, name: ct )
       end
     end
   end
