...
ChargeCategory.where(code: %w(cargo CARGO), tenant_id: nil).each do |charge_category|
  charge_category.name = 'Freight'
  charge_category.save!
end

ChargeCategory.where(code: %w(import IMPORT), tenant_id: nil).each do |charge_category|
  charge_category.name = 'Destination Local Charges'
  charge_category.save!
end

ChargeCategory.where(code: %w(export EXPORT), tenant_id: nil).each do |charge_category|
