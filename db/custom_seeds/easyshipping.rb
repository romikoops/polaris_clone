...
subdomains.each do |sub|
  ...
  hubs = 'data/easyshipping/easyshipping__hubs.xlsx'
  req = { 'key' => hubs }
  ExcelTool::HubsOverwriter.new(params: req, _user: shipper).perform

  public_pricings = 'data/easyshipping/easyshipping__freight_rates.xlsx'
  ...
  local_charges = 'data/easyshipping/easyshipping__local_charges.xlsx'
  ...
  trucking = 'data/easyshipping/easyshipping__trucking_ltl__copenhagen_port.xlsx'
  req = { 'key' => trucking }
  ...
  puts 'Copenhagen Port'
  hub = tenant.hubs.find_by_name('Copenhagen Port')
  trucking = 'data/easyshipping/easyshipping__trucking_ftl__copenhagen_port.xlsx'
  req = { 'key' => trucking }
  # overwrite_zonal_trucking_rates_by_hub(req, shipper, hub.id)
  ExcelTool::OverrideTruckingRateByHub.new(params: req, _user: shipper, hub_id: hub.id).perform
  puts 'Copenhagen Railyard'
