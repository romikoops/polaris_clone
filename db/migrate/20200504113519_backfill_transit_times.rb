    ...
    safety_assured do
      execute("
        ...
        INSERT INTO legacy_transit_times(itinerary_id, tenant_vehicle_id, duration, created_at, updated_at)
             SELECT unique_trips.itinerary_id, unique_trips.tenant_vehicle_id, DATE_PART('day', unique_trips.start_date - unique_trips.end_date), current_timestamp, current_timestamp
             FROM unique_trips
            ")
    end
  end
