...
RSpec.describe RateExtractor::MarginFilter do
  ...
  context "when filtered by validity" do
    let!(:valid_margins) {
      FactoryBot.create_list(:rates_margin, 2, organization: organization, applicable_to: organization, validity: 2.days.from_now..60.days.from_now)
    }

    let!(:invalid_margins) {
      FactoryBot.create_list(:rates_margin, 2, organization: organization, applicable_to: organization, validity: 3.days.ago..2.days.ago)
    }
