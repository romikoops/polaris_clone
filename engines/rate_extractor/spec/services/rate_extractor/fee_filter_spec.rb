...
RSpec.describe RateExtractor::FeeFilter do
  ...
  context "when cargo is LCL" do
    ...
    let(:klass) {
      described_class.new(consolidation: true, desired_date: 1.month.from_now,
                          cargo_rate: cargo_rate, section_rate: section_rate, cargo: decorated_cargo)
    }
    ...
    it "filters out the unapplicable fees" do
      expected_fees = [
        kg_based_fee, unit_based_fee, km_based_fee, cbm_based_fee, wm_based_fee, stowage_based_fee
      ].pluck(:id)
