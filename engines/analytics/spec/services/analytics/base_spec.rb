...
RSpec.describe Analytics::Base, type: :service do
  ...
  let(:service) { described_class.new(user: user, organization: organization, start_date: start_date, end_date: end_date) }
  let(:itineraries) do
    mots.map do |mot|
  ...
  before do
    ...
    FactoryBot.create(:organizations_scope, target: organization,  content: { blacklisted_emails: [blacklisted_client.email]  })
  end
  ...
  context 'when a quote shop' do
    before do
      Organizations::Scope.find_by(target: organization).update(content: { closed_quotation_tool: true, blacklisted_emails: [blacklisted_client.email] })
    end
  ...
  context 'when a booking shop' do
    before do
      Quotations::Tender.find_each do |tender|
        ::Organizations.current_id = organization.id
        FactoryBot.create(:shipments_shipment_request, user: tender.quotation.user, organization: organization, tender: tender)
      end
    end
