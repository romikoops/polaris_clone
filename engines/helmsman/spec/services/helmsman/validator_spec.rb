...
RSpec.describe Helmsman::Validator do
  context "validating routes for the target organization" do
    describe "perform" do
        ...
        hub_locations.permutation(2).flat_map do |loc_array|
          %w[air truck rail ocean].flat_map do |mot|
            FactoryBot.create(:routing_route,
              ...
              origin_terminal: hub_terminals.find { |ter|
                ter.location_id == loc_array.first.id && ter.mode_of_transport == mot
              },
              destination_terminal: hub_terminals.find { |ter|
                ter.location_id == loc_array.last.id && ter.mode_of_transport == mot
              },
      ...
      let!(:ocean_line_service) {
        FactoryBot.create(:routing_line_service, name: "OCEAN - line 1", carrier: ocean_carrier)
      }
      let!(:ocean_line_service_2) {
        FactoryBot.create(:routing_line_service, name: "OCEAN - line 2", carrier: ocean_carrier)
      }
      let!(:air_line_service) { FactoryBot.create(:routing_line_service, name: "AIR - line 1", carrier: air_carrier) }
      let!(:carriage_line_service) {
        FactoryBot.create(:routing_line_service, name: "CARRIAGE - line 1", carrier: carriage_carrier)
      }
      ...
      let!(:ocean_route_line_service) {
        FactoryBot.create(:routing_route_line_service, route: ocean_route, line_service: ocean_line_service)
      }
      let!(:air_route_line_service) {
        FactoryBot.create(:routing_route_line_service, route: air_route, line_service: air_line_service)
      }
      let!(:de_carriage_route_line_service) {
        FactoryBot.create(:routing_route_line_service, route: de_trucking_route, line_service: carriage_line_service)
      }
      let!(:cn_carriage_route_line_service) {
        FactoryBot.create(:routing_route_line_service, route: cn_trucking_route, line_service: carriage_line_service)
      }
      ...
      it "finds four valid routes" do
        FactoryBot.create(
          :routing_route_line_service, route: ocean_route, line_service: ocean_line_service_2
        )
