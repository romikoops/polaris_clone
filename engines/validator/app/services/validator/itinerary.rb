...
module Validator
  class Itinerary
    attr_reader :itinerary, :origin_hub, :destination_hub, :date_range,
      :tenant_vehicle_lookup, :organization, :scope
    ...
    def group_ids
      groups.ids | [default_group.id]
    end

    def tenant_vehicles
      @tenant_vehicle_lookup = group_ids.each_with_object(Hash.new { |h, k| h[k] = {} }) { |group_id, hash|
    ...
    def path_results
      tenant_vehicle_lookup.map do |group_id, tenant_vehicle_and_cargo_classes|
        results = tenant_vehicle_and_cargo_classes.map { |tenant_vehicle_id, cargo_class|
    ...
    def group_info(group_id:)
      Groups::Group.find(group_id)&.slice(:name, :id)
    end
