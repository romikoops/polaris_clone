...
module Wheelhouse
  RSpec.describe QueryParamTransformationService, type: :service do
    ...
    let(:reformatted_params) { described_class.new(params: siren_params).perform }
    ...
    describe "#perform" do
      ...
      context "when lcl" do
        it "correctly processes the params", :aggregate_failures do
          expect(reformatted_params[:cargo_items_attributes]).to be_present
          expect(reformatted_params[:selected_day]).to eq(cargo_ready_date)
          expect(reformatted_params.dig(:origin, :nexus_id)).to eq(itinerary.origin_hub.nexus_id)
          expect(reformatted_params.dig(:destination, :nexus_id)).to eq(itinerary.destination_hub.nexus_id)
        end
      end
      ...
      context "when cargo_ready_date is not provided" do
        ...
        it "uses todays date as the fallback" do
          expect(reformatted_params[:selected_day]).to eq(Time.zone.today.to_s)
        end
      end
      ...
      context "when Aggregated Cargo Item" do
        ...
        it "correctly processes the params", :aggregate_failures do
          expect(reformatted_params[:aggregated_cargo_attributes]).to match(
      ...
      context "when fcl_20" do
        ...
        it "correctly handles fcl param and sets the cargo class", :aggregate_failures do
          expect(reformatted_params[:containers_attributes].pluck(:cargo_class)).to eq(["fcl_20"])
          expect(reformatted_params.dig(:origin, :nexus_id)).to eq(itinerary.origin_hub.nexus_id)
          expect(reformatted_params.dig(:destination, :nexus_id)).to eq(itinerary.destination_hub.nexus_id)
        end
      end
