...
module AdmiraltyReports
  RSpec.describe Stats, type: :model do
    ...
    let(:legacy_quotations_updated_date) { Date.new(2019, 2, 3) }

    context 'when stat is created for quote shop' do
      before do
        FactoryBot.create(:quotations_quotation, tenant: tenant, updated_at: updated_date, created_at: DateTime.new(2020, 2, 1), user: user)
        FactoryBot.create(:tenants_scope, target: tenant, content: { open_quotation_tool: true })
      end

      let(:expected_stat) do
        { updated_date => { combined_data:
                                                { avg_time_for_booking_process: '2 days',
    ...
    context 'when stat is created for non-quote shop' do
      before do
        FactoryBot.create(:legacy_shipment, user: user, tenant: legacy_tenant, updated_at: updated_date, created_at: Date.new(2020, 2, 1), status: 'confirmed')
    ...
    context 'when stat is created for quote shop before quotation quotes' do
      before do
        legacy_shipment = FactoryBot.create(:legacy_shipment, user: user, tenant: legacy_tenant, updated_at: legacy_quotations_updated_date, created_at: Date.new(2019, 2, 2), status: 'confirmed')
        FactoryBot.create(:legacy_quotation, user: user, updated_at: legacy_quotations_updated_date, created_at: Date.new(2019, 2, 2), original_shipment_id: legacy_shipment.id)
        FactoryBot.create(:tenants_scope, target: tenant, content: { open_quotation_tool: true })
      end

      let(:expected_stat) do
        { legacy_quotations_updated_date => { combined_data:
                                                { avg_time_for_booking_process: '1 day',
                                                  n_individual_agents: 1,
                                                  n_shipments: 1 },
                                              data_per_agent: [{ agency_name: nil,
                                                                 count: 1,
