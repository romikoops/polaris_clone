...
module AdmiraltyReports
  RSpec.describe Stats, type: :model do
    ...
    context "when stat is created for quote shop before quotation quotes" do
      before do
        ::Organizations.current_id = organization.id
        legacy_shipment = FactoryBot.create(:legacy_shipment,
          user: user,
          organization: organization,
          updated_at: legacy_quotations_updated_date,
          created_at: legacy_quotations_created_date,
          status: "confirmed")
        FactoryBot.create(:legacy_quotation,
          user: user,
          updated_at: legacy_quotations_updated_date,
          created_at: legacy_quotations_created_date,
          original_shipment_id: legacy_shipment.id)
        FactoryBot.create(:organizations_scope, target: organization, content: {open_quotation_tool: true})
        FactoryBot.create(:companies_company, :with_member, name: "company", organization: organization, member: user)
      end
