      ...
      def query_lookup
        {
          ...
          routes: {
            ...
            insert_schema: 'origin_id, destination_id, allowed_cargo, mode_of_transport, time_factor, price_factor,',
            insert_select: 'ol.id, dl.id, li.allowed_cargo, li.mode_of_transport, li.time_factor, li.price_factor,',
            entry_clauses: ' left join routing_locations ol on ol.name = li.origin_name
            left join routing_locations dl on dl.name = li.destination_name',
            on_conflict: 'do nothing'
          },
          transit_times: {
            ...
            import_schema: 'service_level character varying, carrier_name character varying, days decimal,
            origin_name character varying, destination_name character varying',
            ...
            entry_clauses: 'left join routing_locations ol on ol.name = li.origin_name
            left join routing_locations dl on dl.name = li.destination_name
            left join routing_routes r on r.origin_id = ol.id and r.destination_id = dl.id
            left join routing_carriers c on c.abbreviated_name = li.carrier_name
            left join routing_line_services ls on ls.name = li.service_level and ls.carrier_id = c.id
