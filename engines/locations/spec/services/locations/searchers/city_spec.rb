...
RSpec.describe Locations::Searchers::City do
  ...
  describe ".data" do
    let!(:location1) { FactoryBot.create(:swedish_location, osm_id: 1, admin_level: 7) }
    let!(:location2) { FactoryBot.create(:xl_swedish_location, admin_level: 6) }
    ...
    before do
      FactoryBot.create(:locations_name,
        :reindex,
        location: location2,
        point: location2.bounds.centroid,
        city: "Gothenburg",
        country_code: "SE",
        name: "Vastra Volunda",
        place_rank: 50)
      FactoryBot.create(:locations_name,
        :reindex,
        osm_id: 2,
        point: location1.bounds.centroid,
        city: "Gothenburg",
        country_code: "SE",
        name: "Port 4",
        place_rank: 80)
    ...
    it "finds the Name and returns the attached location" do
      expect(result).to eq(location2.id)
    end
