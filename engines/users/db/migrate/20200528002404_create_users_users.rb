...
class CreateUsersUsers < ActiveRecord::Migration[5.2]
  def change
    create_table :users_users, id: :uuid do |t|
      ...
      t.index %i[last_logout_at last_activity_at], name: :users_users_activity, where: "deleted_at IS NULL"

      t.index %i[email organization_id], unique: true
      t.index %i[email type], unique: true, where: "organization_id IS NULL"
    end
  end
end
