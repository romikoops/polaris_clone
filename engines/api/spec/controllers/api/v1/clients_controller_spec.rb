...
module Api
  RSpec.describe V1::ClientsController, type: :controller do
    ...
    describe "POST #create" do
      let(:user_info) {
        FactoryBot.attributes_for(:organizations_user, organization_id: organization.id)
      ...
      let(:request_object) do
        post :create, params: {
          organization_id: organization.id, client: {**user_info, **profile_info, **address_info}
        }, as: :json
      end
      let(:user_groups) {
      ...
      context "when creating client with email belonging to a soft deleted user" do
        let(:user) {
          FactoryBot.create(:organizations_user, :with_profile, organization: organization, email: "email123@demo.com")
        }
