...
RSpec.describe OfferCalculator::Service::Charges::ChargeConsolidation do
  ...
  describe "#perform" do
    ...
    context "when there a single AggregateLCL CargoUnit present and the fee is lcl based" do
      ...
      it "matches `aggregate_lcl` to `lcl`" do
        expect(charges.map(&:cargo_class)).to eq(["lcl"])
      end
    end

    context "when there are multiple similar cargo classes" do
      ...
      it "matches the cargo class of the context" do
    ...
    context "when there are no fees and only a margin" do
      ...
      it "parses the margin as a fee" do
        expect(charges.map(&:cargo_class)).to eq(["lcl"])
      end
    end
  end
