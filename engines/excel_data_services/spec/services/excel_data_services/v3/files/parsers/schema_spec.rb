...
RSpec.describe ExcelDataServices::V3::Files::Parsers::Schema do
  ...
  describe "#perform" do
    ...
    it "raises and InvalidPath error when the path is invalid" do
      expect { described_class.new(section: section, pattern: pattern, path: "blue") }.to raise_error(ExcelDataServices::V3::Files::Parsers::Schema::InvalidPath)
    end

    it "raises and InvalidSection error when the path is invalid" do
      expect { described_class.new(section: "blue", pattern: pattern, path: path) }.to raise_error(ExcelDataServices::V3::Files::Parsers::Schema::InvalidSection)
    end

    it "raises and InvalidPattern error when the path is invalid" do
      expect { described_class.new(section: section, pattern: "blue", path: path) }.to raise_error(ExcelDataServices::V3::Files::Parsers::Schema::InvalidPattern)
    end
  end
