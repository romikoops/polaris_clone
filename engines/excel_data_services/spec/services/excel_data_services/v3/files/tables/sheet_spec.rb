...
RSpec.describe ExcelDataServices::V3::Files::SheetType do
  ...
  describe "#perform (unit)" do
    let(:pipeline) { instance_double("ExcelDataServices::V3::Files::Section", perform: dummy_state) }
    let(:pipelines) { [pipeline] }
    let(:dummy_state) { instance_double("ExcelDataServices::V3::State", errors: [], stats: [{}]) }
    ...
    context "when an error occurs on a single pipeline" do
      let(:error_state) { instance_double("ExcelDataServices::V3::State", errors: ["There was an error"], stats: [{}]) }
      let(:error_pipeline) { instance_double("ExcelDataServices::V3::Files::Section", perform: error_state) }
  ...
  describe "#state" do
    ...
    it "returns a V3::State object with the correct Overrides defined", :aggregate_failures do
      expect(state).to be_a(ExcelDataServices::V3::State)
