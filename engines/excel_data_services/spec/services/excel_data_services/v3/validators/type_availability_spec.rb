...
RSpec.describe ExcelDataServices::V3::Validators::TypeAvailability do
  include_context "V3 setup"

  let(:result) { described_class.state(state: state_arguments) }
  let(:extracted_table) { result.frame }
  let(:country) { FactoryBot.create(:legacy_country) }
  let(:query_method) { ExcelDataServices::V3::Extractors::QueryMethod::QUERY_METHOD_ENUM["distance"] }
