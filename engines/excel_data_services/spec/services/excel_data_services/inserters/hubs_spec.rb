...
RSpec.describe ExcelDataServices::Inserters::Hubs do
  describe ".perform" do
    ...
    it "creates the correct number of hubs" do
      ...
      expect(stats.dig(:"legacy/nexus", :number_created)).to be(2)
      ...
      expect(Legacy::Nexus.where(organization_id: organization.id).count).to be(2)
    ...
    context "with existing hubs" do
      ...
      it "creates the correct number of hubs and updates the rest" do
        aggregate_failures do
          ...
          expect(stats.dig(:"legacy/nexus", :number_created)).to be(1)
          expect(stats.dig(:"legacy/addresses", :number_created)).to be(2)
          expect(hubs.count).to be(2)
          expect(hubs.pluck(:mandatory_charge_id)).to match_array(mandatory_charges.pluck(:id))
          expect(Legacy::Nexus.where(organization_id: organization.id).count).to be(2)
    ...
    context "with existing hubs wiht same locode, diff mot" do
      ...
      it "creates the correct number of hubs and updates the rest" do
        aggregate_failures do
          ...
          expect(Legacy::Nexus.where(organization_id: organization.id, locode: "AUADL").count).to be(1)
        end
      end
    end
