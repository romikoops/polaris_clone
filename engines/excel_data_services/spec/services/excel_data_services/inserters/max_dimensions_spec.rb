...
RSpec.describe ExcelDataServices::Inserters::MaxDimensions do
  ...
  let(:carrier) { FactoryBot.create(:legacy_carrier, code: "msc", name: "msc") }
  let(:applicable) { tenant }
  let(:options) { { organization: organization, data: input_data, options: {} } }

  before do
    FactoryBot.create(:legacy_tenant_vehicle, organization: organization, name: "standard", carrier: carrier)
    FactoryBot.create(:legacy_tenant_vehicle, organization: organization, name: "faster", carrier: carrier)
    FactoryBot.create(:gothenburg_shanghai_itinerary, organization: organization)
  end

  describe ".insert" do
    let(:input_data) { FactoryBot.build(:excel_data_restructured_max_dimensions) }
