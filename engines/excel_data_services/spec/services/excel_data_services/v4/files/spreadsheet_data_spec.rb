...
RSpec.describe ExcelDataServices::V4::Files::SpreadsheetData do
  ...
  let(:section_parser) { instance_double("ExcelDataServices::V4::Files::SectionParser", sheets: sheets) }
  let(:sheets) { ["Sheet1"] }
  let(:sheet_object_double) { instance_double(ExcelDataServices::V4::Files::Tables::Sheet, errors: errors, perform: sheet_object_frame) }
  let(:sheet_object_frame) { Rover::DataFrame.new({ "a" => [1] }) }

  before do
    allow(ExcelDataServices::V4::Files::Tables::Sheet).to receive(:new).with(state: state_arguments, sheet_name: "Sheet1", section_parser: section_parser).and_return(sheet_object_double)
  end
  ...
  describe "#frame" do
    context "when there are no errors" do
      ...
      context "when there are multiple Sheet objects with no errors" do
        let(:second_sheet_object_double) { instance_double("ExcelDataServices::V4::Files::Sheet", errors: errors, perform: second_sheet_object_frame) }
        let(:second_sheet_object_frame) { Rover::DataFrame.new({ "a" => [2] }) }
        let(:sheets) { %w[Sheet1 Sheet2] }

        before do
          allow(ExcelDataServices::V4::Files::Tables::Sheet).to receive(:new).with(state: state_arguments, sheet_name: "Sheet2", section_parser: section_parser).and_return(second_sheet_object_double)
        end
