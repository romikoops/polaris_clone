...
RSpec.describe ExcelDataServices::V4::Files::ActionExecutor do
  ...
  let(:action_double) { class_double(ExcelDataServices::V4::Formatters::Pricing) }
  ...
  describe "#perform" do
    ...
    context "when there are no errors" do
      ...
      context "when there are two Actions" do
        let(:second_action_double) { class_double(ExcelDataServices::V4::Formatters::Pricing) }
    ...
    context "when there are errors" do
      ...
      context "when there are two Actions and the first returns an error" do
        let(:second_action_double) { class_double(ExcelDataServices::V4::Formatters::LocalCharge) }
        let(:service) { described_class.new(state: state_arguments, actions: [action_double, second_action_double]) }
