...
RSpec.describe ExcelDataServices::Restructurers::SacoShipping do
  ...
  describe ".restructure" do
    ...
    let(:output_data_pricings) { {"Pricing" => FactoryBot.build(:excel_data_restructured_correct_saco_shipping_pricings)} }
    let(:output_data_local_charges) { {"LocalCharges" => FactoryBot.build(:excel_data_restructured_correct_saco_shipping_local_charges)} }
