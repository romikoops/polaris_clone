# frozen_string_literal: true

require "rails_helper"

RSpec.describe ExcelDataServices::DataFrames::Importers::TypeAvailabilities do
  let(:organization) { FactoryBot.create(:organizations_organization) }
  let(:country) { FactoryBot.create(:legacy_country) }
  let(:load_type) { "cargo_item" }
  let(:truck_type) { "default" }
  let(:carriage) { "pre" }
  let(:data) do
    Rover::DataFrame.new([{
      query_method: "location",
      country_id: country.id,
      load_type: load_type,
      truck_type: truck_type,
      carriage: carriage
    }])
  end
  let(:options) { { organization: organization, data: data, options: {} } }
  let(:stats) { described_class.import(data: data, type: "type_availabilities") }

  describe ".import" do
    context "when no hub availabilities exist" do
      it "successfuly imports the data" do
        expect(stats.created).to eq(1)
      end
    end

    context "when hub availabilities exist" do
      before do
        FactoryBot.create(:trucking_type_availability,
          query_method: "location",
          country_id: country.id,
          load_type: load_type,
          truck_type: truck_type,
          carriage: carriage)
      end

      it "successfuly upserts the data" do
        expect(stats.created).to eq(0)
      end
    end
  end
end
