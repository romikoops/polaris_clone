# frozen_string_literal: true

require "rails_helper"

module ExcelDataServices
  RSpec.shared_examples "rate_basis validator", type: :service do
    let(:exception) { ExcelDataServices::Validators::ValidationErrors::TypeValidity::RateBasisType }

    context "with valid string input" do
      let(:rate_basis_value) { "PER_SHIPMENT" }

      it_behaves_like "passing validator"
    end

    context "with invalid string input" do
      let(:rate_basis_value) { "horse" }

      it_behaves_like "failing validator"
    end

    context "with invalid float input" do
      let(:rate_basis_value) { 1.0 }

      it_behaves_like "failing validator"
    end

    context "with invalid integer input" do
      let(:rate_basis_value) { 1 }

      it_behaves_like "failing validator"
    end

    context "with invalid nil inputs" do
      let(:rate_basis_value) { nil }

      it_behaves_like "failing validator"
    end
  end
end
