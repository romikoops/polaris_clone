...
RSpec.describe Pricings::Manipulator do
  ...
  before do
    ...
    FactoryBot.create(:organizations_scope, content: { base_pricing: true }, target: organization)
  ...
  describe '.perform' do
    ...
    context 'with multiple manipulated freight pricings when margins overlap attached to the group without pricing' do
      Timecop.freeze(Time.zone.now) do
        let!(:margin_a) do
          FactoryBot.create(:freight_margin, pricing: pricing, effective_date: Time.zone.today - 3.days, expiration_date: (Time.zone.today + 10.days).end_of_day, organization: organization, applicable: group)
        end
        let!(:margin_b) do
          FactoryBot.create(:freight_margin,
                            pricing: pricing,
    ...
    context 'with manipulated freight pricing with specific detail attached to the user' do
      let(:pricing) do
        FactoryBot.create(:pricings_pricing, tenant_vehicle: tenant_vehicle, organization: organization, itinerary: itinerary).tap do |tapped_pricing|
          FactoryBot.create(:fee_per_wm, organization: organization, charge_category: bas_charge_category, pricing: tapped_pricing, rate: 40)
    ...
    context 'with manipulated freight pricing with one specific detail and general attached to the user' do
      let(:pricing) do
        FactoryBot.create(:pricings_pricing, tenant_vehicle: tenant_vehicle, itinerary: itinerary).tap do |tapped_pricing|
          FactoryBot.create(:fee_per_wm, organization: organization, charge_category: bas_charge_category, pricing: tapped_pricing, rate: 40)
          FactoryBot.create(:fee_per_wm, organization: organization, charge_category: baf_charge_category, pricing: tapped_pricing, rate: 40)
          FactoryBot.create(:freight_margin, pricing: tapped_pricing, organization: organization, applicable: user).tap do |tapped_margin|
            FactoryBot.create(:bas_margin_detail, margin: tapped_margin, value: 0.25, charge_category: bas_charge_category)
          end
        end
