...
FactoryBot.define do
  factory :ledger_rate, class: "Ledger::Rate" do
    ...
    trait :everything_lcl do
      after(:build) do |rate, evaluator|
        %i[range_kg_fee
          ...
          rate.fees << build(
            key, :cargo_item, varied_dates: evaluator.varied_dates, start: evaluator.start, random: evaluator.random
          )
        end
      end
    end
    ...
    trait :everything_fcl do
      after(:build) do |rate, evaluator|
        %i[range_kg_fee
          ...
          rate.fees << build(
            key, :container_20, varied_dates: evaluator.varied_dates, start: evaluator.start, random: evaluator.random
          )
        end
      end
    end
    ...
    trait :air_default do
      after(:build) do |rate, evaluator|
        rate.fees << build(
          :range_kg_fee, varied_dates: evaluator.varied_dates, start: evaluator.start, random: evaluator.random
        )
      end
    end
    ...
    trait :fcl_default do
      after(:build) do |rate, evaluator|
        rate.fees << build(
          :container_fee, varied_dates: evaluator.varied_dates, start: evaluator.start, random: evaluator.random
        )
      end
    end
