      ...
      private
        ...
        def determine_local_charge_type
          ...
          if @hub_data[:routing] && @hub_data[:routing].include?('RTM') && @direction == 'import'
            rtm_charge
          elsif @hub_data[:country] && 'Japan' == @hub_data[:country] && @direction == 'export'
            japan_charge
          elsif @hub_data[:country] && ('United States of America' == @hub_data[:country] || 'Usa' == @hub_data[:country]) && @direction == 'export'
            us_charge
          else
            LocalCharge.new
          end
