  ...
  module DataValidators
    class HeaderChecker < Base
      ...
      def perform
        ...
        unless diff.blank? # rubocop:disable Style/GuardClause
          add_to_errors(
            ...
            exception_class: ExcelDataServices::DataValidators::ValidationErrors::HeaderChecker
          )
        end
      end
      ...
      def data_restructurer_names
        self.class::StaticHeadersForDataRestructurers.constants(false).map { |constant| constant.to_s.downcase }
      end

      def headers_from_data_restructurer_name(restructurer_name)
        "#{self.class}::StaticHeadersForDataRestructurers::#{restructurer_name.upcase}".constantize
      end
    end
  end
end
