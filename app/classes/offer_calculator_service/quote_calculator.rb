...
module OfferCalculatorService
  class QuoteCalculator < Base
    include CurrencyTools
    include PricingTools

    def initialize(args={})
      @trucking_data = args[:trucking_data]
      @faux_schedule      = args[:schedule]
      @user          = args[:user]
      
      @tenant_vehicle = @faux_schedule.trip.tenant_vehicle
      super(args[:shipment])
    end
    ...
    def perform
      ...
      @charge_breakdown = ChargeBreakdown.create!(shipment: @shipment, trip_id: @faux_schedule.trip_id)
    ...
    def calc_local_charges
      ...
      if @shipment.has_pre_carriage || @faux_schedule.origin_hub.mandatory_charge.export_charges
        local_charges_data = determine_local_charges(
          @faux_schedule.origin_hub,
          @shipment.load_type,
          cargo_units,
          "export",
          @faux_schedule.mode_of_transport,
          @faux_schedule.trip.tenant_vehicle.id,
          @faux_schedule.destination_hub_id,
          @user
        )
      ...
      if @shipment.has_on_carriage || @faux_schedule.destination_hub.mandatory_charge.import_charges
        local_charges_data = determine_local_charges(
          @faux_schedule.destination_hub,
          @shipment.load_type,
          cargo_units,
          "import",
          @faux_schedule.mode_of_transport,
          @faux_schedule.trip.tenant_vehicle.id,
          @faux_schedule.origin_hub_id,
          @user
        )
    ...
    def create_trucking_charges
      @trucking_data.each do |carriage, data|
        ...
        hub = @faux_schedule.hub_for_carriage(carriage)
    ...
    def calc_cargo_charges
      ...
      if @user.tenant.scope["consolidate_cargo"] && cargo_unit_array.first.is_a?(CargoItem)
        cargo_unit_array = consolidate_cargo(cargo_unit_array, @faux_schedule.mode_of_transport)
      end
    ...
    def destroy_previous_charge_breakdown
      ChargeBreakdown.find_by(shipment: @shipment, trip_id: @faux_schedule.trip_id).try(:destroy)
    end
