...
module ExcelTool
  class RawIgs < ExcelTool::BaseTool
    ...
    def sort_data
      ...
      name_only_data.each do |name, name_values|
        if name_values.length > 1
          ...
          # values
          #   .group_by {|v| [v[:fcl_20][:small], v[:fcl_20][:medium], v[:fcl_20][:large]]}
          #   .each do |group_key, grouped_values|
          #     if grouped_values.length > 1
          #       values.each {|v| name_only_row_nrs << v[:row_nr]}
          #     end
          #   end
        end 
      end
    ...
    def write_data
        ...
        @workbook.close
        # write_to_aws(@directory, Tenant.schyrver, @filename, "sorted_trucking_data")
    end
