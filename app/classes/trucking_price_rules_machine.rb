class TruckingPriceRulesMachine
  ...
  def formula_price(pricing, km, weight_in_tons, volume_in_cm3, units)
    ...
    price2 = volume_in_cm3 / 1000000 * pricing.price_per_m3 * units + km * pricing.price_per_km

    price1 > price2 ? total_price = price1 : total_price = price2
  end
  ...
  def steptable_price(pricing, weight_in_tons, volume_in_cm3, units, zip_code)
    ...
    volume = volume_in_cm3 / 1000000 * units
    ...
    table.keys.each do |zip_code_range_string|
      ...
      if zip_code.between?(zip_code_range_array.first, zip_code_range_array.last)
