...
class PdfHandler
  ...
  def calculate_cargo_data(shipment)
    ...
    when 'weight'
      ...
      cargo_string = " (Chargeable Weight: #{'%.2f' % chargeable_weight_cargo} kg)"
      shipment.cargo_units.each do |hash|
        string = "#{'%.2f' % hash[:chargeable_weight]} kg"
    ...
    when 'volume'
      ...
      cargo_string = " (Chargeable Volume: #{'%.3f' % chargeable_weight_cargo} m<sup>3</sup>)"
      shipment.cargo_units.each do |hash|
        string = "#{'%.3f' % (hash[:chargeable_weight] / 1000)} m<sup>3</sup>"
        chargeable_weight[hash[:id].to_s] = {
          value: string,
    ...
    when 'dynamic'
      ...
      shipment.cargo_units.each do |hash|
        string = if show_volume 
          "#{'%.3f' % (hash[:chargeable_weight] / 1000)} m<sup>3</sup>"
        else
          "#{'%.2f' % hash[:chargeable_weight]} kg"
        end
        chargeable_weight[hash[:id].to_s] = {
          value: string,
