module SeedTools
  def overwrite_main_carriage_rates(params, dedicated)
    ...
    pricing_rows.each do |row|
      ...
      route = Route.find_or_create_by(name: "#{origin.name} - #{destination.name}", tenant_id: seed_user.tenant_id, origin_nexus_id: origin.id, destination_nexus_id: destination.id)
  ...
  def overwrite_trucking_rates(params)
    ...
    xlsx.sheets.each do |sheet_name|
      ...
      (3..num_rows).each do |line|
        ...
        ntp = TruckingPricing.new(currency: currency_row[3], tenant_id: seed_user.tenant_id, nexus_id: nexus.id, lower_zip: zip_code_range_array[0].to_i, upper_zip: zip_code_range_array[1].to_i)
  ...
  def overwrite_service_charges(params)
    ...
    rows.each do |r|
      ...
      hub = Hub.find_by("location_id = ? AND tenant_id = ?", og.id, seed_user.tenant_id)
  ...
  def overwrite_train_schedules(params)
    ...
    schedules.each do |train_schedule|
      ...
      if robj
        ...
        nrt = Route.create_from_schedule(train_schedule, seed_user.tenant_id)
  ...
  def overwrite_hubs(params)
    ...
    hub_rows.each do |hub_row|
      ...
      hub = nexus.hubs.find_or_create_by(hub_code: hub_code, location_id: nexus.id, tenant_id: seed_user.tenant_id, hub_type: hub_row[:hub_type], latitude: hub_row[:latitude], longitude: hub_row[:longitude], name: "#{nexus.name} #{hub_type_name[hub_row[:hub_type]]}")
      new_ids << hub.id
    end
