...
module DataWriter
  class LocalChargesWriter < BaseWriter
    ...
    def build_row_data(local_charge, fee)
      # TODO: "port" should rather be "hub"
      port = tenant.hubs.find(local_charge.hub_id)
      port_name = remove_hub_suffix(port.name, port.hub_type)
      country_name = port.address.country.name
