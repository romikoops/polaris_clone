...
class CurrenciesController < ApplicationController
  ...
  def get_currencies_for_base
    results = get_currency_array(params[:currency], params[:tenant_id] || current_user.tenant_id)
    response_handler(results)
  end
  ...
  def set_currency
    ...
    current_user.save!
    rates = get_rates(params[:currency], params[:tenant_id] || current_user.tenant_id)
