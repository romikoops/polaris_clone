...
module Admin
  class MarginsController < Admin::AdminBaseController
    def index
    ...
    def update_multiple
      updated_margins = bulk_update_params[:margins].map do |param_margin|
        ...
        if param_margin[:marginDetails].present?
          param_margin[:marginDetails].each do |param_margin_detail|
            margin.details.find(param_margin_detail[:id]).update(operator: param_margin_detail[:operator], value: param_margin_detail[:value])
          end
        end
    ...
    def form_data
      ...
      if params[:itinerary_id].present?
        ...
        itinerary.rates.each do |pr|
          hash[:cargo_classes].push(cargo_class: pr.cargo_class, itinerary_id: itinerary.id) unless hash[:cargo_classes].include?(cargo_class: pr.cargo_class, itinerary_id: itinerary.id)
    ...
    def fee_data
      result = case params[:margin_type]
               when "local_charges"
                 local_charge_fees
               when "trucking"
                 trucking_fees
    ...
    def local_charge_fees
      local_charges =
        Legacy::LocalCharge.where(
          ...
          counterpart_hub_id: params[:counterpart_hub_id] == "null" ? nil : params[:counterpart_hub_id],
          tenant_vehicle_id: extract_tenant_vehicle_ids,
