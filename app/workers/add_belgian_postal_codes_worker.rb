...
class AddBelgianPostalCodesWorker
  include Sidekiq::Worker
  include Sidekiq::Status::Worker

  BUCKET = "itsmycargo-datahub"
  PATH = "production/maps/belgium_postal_district_boundaries.geojson"
  ...
  def perform
    ...
    postal_codes.each_with_index do |postal_code, index|
      ...
      next if postal_code_name.blank?

      existing_location = Locations::Location.find_by(country_code: "be", name: postal_code_name)
      if existing_location
        existing_location.update!(bounds: postal_bounds, admin_level: nil)
      else
        Locations::Location.create!(name: postal_code_name, country_code: "be", bounds: postal_bounds, admin_level: nil)
      end
    end
  end
  ...
  def postal_code_name_from(bounds:)
    found_postal_code = belgian_postal_code_points.find do |postal_code_with_point|
  ...
  def belgian_postal_code_points
    @belgian_postal_code_points ||= point_s3_data["features"].select { |geo_json| geo_json.dig("properties", "CNTR_ID") == "BE" }
  end
