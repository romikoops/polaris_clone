...
RUN psql -d postgis_template -c "create extension if not exists postgis;"
RUN pg_ctl stop -w

USER root
RUN whoami
# Start PostgreSQL and proceed further
...
# Copy repos content to docker image
RUN mkdir -p /cargo
COPY ./ /cargo
...
# Following two lines are added to overcome docker build bug
...
RUN rake db:drop db:create db:migrate db:seed:geometries db:seed:demo db:seed:trucking_pricing
RUN rails server &
