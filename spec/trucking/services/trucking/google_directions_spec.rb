...
RSpec.describe Trucking::GoogleDirections do
  ...
  let(:url) { "https://maps.googleapis.com/maps/api/directions/xml?alternative=false&departure_time=now&destination=53.536975,9.918213&key=FAKEKEY&language=en&mode=driving&origin=53.5453188,10.000840899999957&traffic_model=pessimistic" }

  before do
    stub_request(:get, url)
      .to_return(status: 200, body: FactoryBot.create(:google_directions_response))
    stub_request(:get, "https://maps.googleapis.com/maps/api/directions/xml?alternative=false&departure_time=now&destination=53.536975,9.918213&key=FAKEKEY&language=en&mode=driving&origin=53.5453188,10.000840899999957&traffic_model=pessimistic")
      .to_return(status: 200, body: FactoryBot.create(:google_directions_response))
    stub_request(:get, "https://maps.googleapis.com/maps/api/directions/xml?alternative=false&departure_time=1576540800&destination=53.536975,9.918213&key=FAKEKEY&language=en&mode=driving&origin=53.5453188,10.000840899999957&traffic_model=pessimistic")
      .to_return(status: 200, body: FactoryBot.create(:google_directions_response))
  end
