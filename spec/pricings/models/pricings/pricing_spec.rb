...
module Pricings
  RSpec.describe Pricing, type: :model do
    context "class methods" do
      ...
      let!(:fcl_40_pricing) {
        FactoryBot.create(:fcl_40_pricing, tenant_vehicle: tenant_vehicle_1, itinerary: itinerary_1)
      }
      let!(:fcl_40_hq_pricing) {
        FactoryBot.create(:fcl_40_hq_pricing, tenant_vehicle: tenant_vehicle_1, itinerary: itinerary_1)
      }
      ...
      describe ".for_cargo_classes" do
        ...
        it "returns the fcl_20 & fcl_40 pricings only" do
          expect(
            ::Pricings::Pricing.for_cargo_classes(%w[fcl_20 fcl_40]).sort
          ).to eq([fcl_20_pricing, fcl_40_pricing].sort)
        end
      end
      ...
      describe ".for_load_type" do
        ...
        it "returns the fcl_20 & fcl_40 pricings only" do
          expect(
            ::Pricings::Pricing.for_load_type("container").sort
          ).to eq([fcl_20_pricing, fcl_40_pricing, fcl_40_hq_pricing].sort)
        end
      end
      ...
      describe ".for_dates" do
        it "returns the lcl pricings only" do
          expect(
            ::Pricings::Pricing.for_dates(Time.zone.today, Time.zone.today + 2.weeks).ids.sort
          ).to eq(
