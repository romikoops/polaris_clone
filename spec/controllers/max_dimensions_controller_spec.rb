...
RSpec.describe MaxDimensionsController, type: :controller do
  ...
  let(:default_params) {
    {organization_id: organization.id, load_type: load_type, itinerary_ids: [itinerary.id].join(",")}
  }
  ...
  describe "GET #index" do
    ...
    context "without carrier mdbs fcl" do
      ...
      let!(:max_dimensions) {
        FactoryBot.create_list(:legacy_max_dimensions_bundle, 1, cargo_class: "fcl_20", organization: organization)
      }
      let!(:max_agg_dimensions) {
        FactoryBot.create_list(:legacy_max_dimensions_bundle, 1,
          cargo_class: "fcl_20", organization: organization, aggregate: true)
      }
