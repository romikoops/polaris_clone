...
RSpec.describe Admin::ShipmentsController, type: :controller do
  ...
  let!(:shipment) {
    FactoryBot.create(:completed_legacy_shipment,
      with_breakdown: true, with_tenders: true, organization: organization,
      user: shipper, status: "requested")
  }
  ...
  describe "GET #index" do
    ...
    context "when a user is nil" do
      let!(:nil_user_shipment) {
        FactoryBot.create(:completed_legacy_shipment,
          with_breakdown: true, with_tenders: true, organization: organization, user: nil, status: "requested")
      }
  ...
  describe "GET #search_shipments" do
    ...
    context "when searching via POL" do
      it "returns matching shipments with origin matching the query" do
        get :search_shipments, params: {
          target: "requested", query: shipment.origin_hub.name, organization_id: organization.id
        }
    ...
    context "when searching via user's company name/Agency" do
      it "returns matching shipments with users matching the company name in query" do
        get :search_shipments, params: {
          target: "requested", query: profile.company_name, organization_id: organization.id
        }
        expect(json.dig(:data, :shipments).first[:id]).to eq(shipment.id)
      end
    end
  ...
  describe "GET #show" do
    before do
      allow(Pricings::Metadatum).to receive(:find_by)
        .and_return(instance_double("Pricings::Metadatum", breakdowns: [breakdown, user_breakdown]))
    end
  ...
  describe "POST #upload_client_document" do
    before do
      post :upload_client_document, params: {
  ...
  describe "POST #document_action" do
    ...
    before do
      post :document_action, params: {
        id: file.id, shipment_id: shipment.id, organization_id: organization.id, type: "approve"
      }
    end
  ...
  describe "POST #update" do
    ...
    before do
      post :update, params: {
        id: shipment.id, organization_id: organization.id, shipment_action: action
      }
    end
  ...
  describe "POST #document_delete" do
    ...
    it "deletes the document" do
      post :document_delete, params: {
        id: file.id, shipment_id: shipment.id, organization_id: organization.id
      }
    ...
    context "when document does not exist" do
      it "returns http status not found" do
        post :document_delete, params: {
          id: "wrong_id", shipment_id: shipment.id, organization_id: organization.id
        }
