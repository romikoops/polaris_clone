...
RSpec.describe Admin::ItinerariesController, type: :controller do
  ...
  let!(:itineraries) do
    [
      ...
      FactoryBot.create(:shanghai_hamburg_itinerary, tenant: tenant, transshipment: 'direct')
    ]
  end
  ...
  describe 'GET #index' do
    ...
    it 'returns http success and index data filtered by transshipment' do
      search_params = {
        transshipment: 'direct',
        transshipment_desc: true
      }.merge(params)
      ...
      json = JSON.parse(response.body)
      expect(json['success']).to eq true
      expect(json.dig('data', 'itinerariesData').length).to eq 1
      expect(json.dig('data', 'itinerariesData').pluck('id')).to match_array(itineraries.last.id)
    end
  end
