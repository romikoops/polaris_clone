...
RSpec.describe TruckingTools, :aggregate_failures do
  let(:fees) do
    {
  ...
  let(:modifier) { 'kg' }
  let(:trucking_pricing) { create(:trucking_trucking, modifier: modifier, rates: rates, fees: fees) }
  let(:cargos) { [create(:cargo_item)] }
  let(:kms) { 42 }
  let(:carriage) { 'pre' }
