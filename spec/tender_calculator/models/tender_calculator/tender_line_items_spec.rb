...
RSpec.describe TenderCalculator::TenderLineItems do
  ...
  let!(:shipment_based_fee) { FactoryBot.create(:rates_fee, :percentage_basis, cargo: shipment_targeted_rate.object, percentage: 0.10) }
  let!(:section_based_fee) { FactoryBot.create(:rates_fee, :percentage_basis, cargo: section_targeted_rate.object, percentage: 0.15) }
  let!(:cargo_based_fee) { FactoryBot.create(:cbm_based_fee, cargo: cargo_targeted_rate.object, amount_cents: 25000) }
  let(:tender_line_items) { described_class.new(section_rates: [section], cargo: decorated_cargo, margins: margins, discounts: discounts) }
  ...
  context 'when percentage margins apply on a cargo rates' do
    let(:margins_arr) {
      FactoryBot.create_list(:rates_margin, 1, target: cargo_targeted_rate.object, operator: :multiplication, percentage: 0.20,
                                               organization: organization, applicable_to: organization)
    }
    let(:margins) { Rates::Margin.where(id: margins_arr.map(&:id)) }
  ...
  context 'when percentage discounts apply on a cargo rates' do
    let(:discounts_arr) {
      FactoryBot.create_list(:rates_discount, 1, :percentage_basis, target: cargo_targeted_rate.object, operator: :multiplication,
                                                                    percentage: 0.20, organization: organization, applicable_to: organization)
    }
    let(:discounts) { Rates::Discount.where(id: discounts_arr.map(&:id)) }
  ...
  context 'when percentage discounts apply on section rates' do
    let(:discounts_arr) {
      FactoryBot.create_list(:rates_discount, 1, :percentage_basis, target: section, operator: :multiplication, percentage: 0.10,
                                                                    organization: organization, applicable_to: organization)
    }
    let(:discounts) { Rates::Discount.where(id: discounts_arr.map(&:id)) }
  ...
  context "when both percentage and flat discounts apply on shipment (no target)" do
    ...
    let(:percentage_discounts) {
      FactoryBot.create_list(:rates_discount, 1, :percentage_basis, operator: :multiplication, percentage: 0.05, order: 0,
                                                                    organization: organization, applicable_to: organization)
    }
  ...
  context "when percentage discounts apply before flats on a rate" do
    let(:flat_discount) {
      FactoryBot.create(:rates_discount, :shipment_basis, target: section, operator: :addition, amount_cents: 100, order: 1,
                                                          organization: organization, applicable_to: organization)
    }
    let(:percentage_discount) {
      FactoryBot.create(:rates_discount, :percentage_basis, target: section, operator: :multiplication, percentage: 0.05, order: 0,
                                                            organization: organization, applicable_to: organization)
    }
  ...
  context "when flat discounts apply before percentages on a rate" do
    let(:flat_discount) {
      FactoryBot.create(:rates_discount, :shipment_basis, target: section, operator: :addition, amount_cents: 100, order: 0,
                                                          organization: organization, applicable_to: organization)
    }
    let(:percentage_discount) {
      FactoryBot.create(:rates_discount, :percentage_basis, target: section, operator: :multiplication, percentage: 0.05, order: 1,
                                                            organization: organization, applicable_to: organization)
    }
    let(:discounts) { Rates::Discount.where(id: [flat_discount.id, percentage_discount.id]) }
