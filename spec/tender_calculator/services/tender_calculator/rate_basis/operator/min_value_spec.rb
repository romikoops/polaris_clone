...
RSpec.describe TenderCalculator::RateBasis::Operator::MinValue do
  ...
  context "when cargo rate operator is min_value" do
    let(:operator) { :min_value }

    it "calculates line item as the minimum value of the multiple children fees" do
      expected_amount = [kg_based_fee.amount * 60, cbm_based_fee.amount * 80].min
