  ...
  describe "#perform" do
    ...
    context "when the charge is of detail level 3" do
      ...
      it "updates grand total of the breakdown" do
        ...
        ).to eq Money.new(5000.0, level_3_charge.price.currency).exchange_to(original_value.currency)
      end
    ...
    context "when there are two charges of the same charge category" do
      ...
      let(:shipment) {
        FactoryBot.create(:legacy_shipment,
          with_tenders: true, organization: organization, with_breakdown: true, containers: [container_1, container_2])
      }
      ...
      let(:level_3_charge) {
        breakdown.charges.find_by(detail_level: 3, children_charge_category: line_item.charge_category)
      }
      ...
      it "does not update the edited price of the charge of unspecified cargo charge" do
        ...
        expect(
          breakdown.charges.where(detail_level: 3, charge_category: unupdated_charge_category).first.edited_price
        ).to be_nil
      end
    end
    ...
    context "when the charge is of detail level 3 and uneditable (included)" do
      ...
      let(:invalid_charge_category) {
        FactoryBot.create(:legacy_charge_categories, organization: organization, code: "included_baf")
      }
      let(:line_item) do
        FactoryBot.create(:quotations_line_item,
          tender: tender,
