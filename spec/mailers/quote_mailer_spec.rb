...
RSpec.describe QuoteMailer, type: :mailer do
  ...
  let(:quotation) do
    FactoryBot.create(:legacy_quotation,
      user: user, original_shipment_id: original_shipment.id, shipment_count: shipment_count)
  end
  ...
  describe "quotation_email (internal)" do
    ...
    let(:quotation) do
      FactoryBot.create(:legacy_quotation,
        user: user, shipment_count: 1, original_shipment: original_shipment, billing: billing)
    end
    let(:mail) {
      described_class.new_quotation_email(
        quotation: quotations_quotation,
    ...
    it "renders", :aggregate_failures do
      expect(
        mail.subject
      ).to eq("FCL Quotation: Gothenburg - Gothenburg, Refs: #{quotations_quotation.tenders.first.imc_reference}")
      expect(mail.from).to eq(["no-reply@#{organization.slug}.itsmycargo.shop"])
      expect(mail.reply_to).to eq(["support@demo.com"])
  ...
  describe "quotation_admin_ email for quotation" do
    ...
    let(:mail) {
      described_class.new_quotation_admin_email(
        quotation: quotations_quotation, shipment: original_shipment
      ).deliver_now
    }
  ...
  describe "quotation_admin_email for shipment" do
    ...
    it "renders", :aggregate_failures do
      expect(
        mail.subject
      ).to eq("FCL Quotation: #{pickup_address.city} - #{delivery_address.city}, "\
        "Refs: #{quotations_quotation.tenders.first.imc_reference}")
      expect(mail.from).to eq(["no-reply@#{organization.slug}.itsmycargo.shop"])
      expect(mail.reply_to).to eq(["support@itsmycargo.tech"])
      expect(mail.to).to eq(["sales.general@demo.com"])
    end
