...
RSpec.describe Validator::Itinerary do
  describe "#perform" do
    ...
    context "with one tenant vehicle chain with invalid Local Charges and no Trips" do
      ...
      it "returns the expected result for one tenant vehicle chain with invalid Local Charges and no Trips", :aggregate_failures do
    ...
    context "with one tenant vehicle chain with invalid Local Charges and good Trips" do
      ...
      it "returns the expected result for one tenant vehicle chain with invalid Local Charges and good Trips", :aggregate_failures do
        expect(target_result.dig(:origin_local_charges)).to eq(required: false, status: "no_data", last_expiry: nil)
        expect(target_result.dig(:destination_local_charges)).to eq(required: false, status: "no_data", last_expiry: nil)
