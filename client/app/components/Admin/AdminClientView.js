...
export class AdminClientView extends Component {
    ...
    assignNewManager() {
        ...
        adminDispatch.assignManager({manager_id: selectedManager.value, role: selectedRole.value, client_id: client.id});
        this.toggleNewManager();
    }
    render() {
        const {theme, clientData, hubs, managers} = this.props;
        if (!clientData) {
            return '';
        }

        const { selectedManager, selectedRole, showAddManager } = this.state;
        const { client, shipments, locations, managerAssignments } = clientData;
        ...
        const managerOpts = managers.map((m) => {
            return {label: `${m.first_name} ${m.last_name}`, value: m.id};
        });
        console.log(managerOpts);
        const relManagers = managerAssignments ? managerAssignments.map((ma) => {
            const man = managers.filter(m => m.id === ma.manager_id)[0];
            man.section = ma.section;
            return man;
        }) : [];
        console.log(managerAssignments);
        const manArray = relManagers ? relManagers.map((ma) => {
            return (
                <div className="flex-100 layout-row layout-align-start-center">
                    ...
                    <div className="flex-50 layout-row layout-align-start-center">
                        <i className="fa fa-book flex-none clip" style={textStyle}></i>
                        ...
                        <div className="flex-65 layout-row layout-align-start-start layout-wrap">
                            <div className="flex-100 layout-row layout-align-start-start layout-wrap">
                                <div className="flex-100 layout-row layout-align-start-start ">
                                    <sup style={textStyle} className="clip flex-none">Company</sup>
                                </div>
                                <div className="flex-100 layout-row layout-align-start-center ">
                            ...
                            <div className="flex-50 layout-row layout-align-start-start layout-wrap">
                                <div className="flex-100 layout-row layout-align-start-start ">
                                    <sup style={textStyle} className="clip flex-none">Email</sup>
                                </div>
                                <div className="flex-100 layout-row layout-align-start-center ">
                            ...
                            <div className="flex-50 layout-row layout-align-start-start layout-wrap">
                                <div className="flex-100 layout-row layout-align-start-start ">
                                    <sup style={textStyle} className="clip flex-none">Phone</sup>
                                </div>
                                <div className="flex-100 layout-row layout-align-start-center ">
