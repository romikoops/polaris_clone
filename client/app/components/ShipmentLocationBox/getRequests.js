...
function findNexus (lat, lng, callback) {
  fetch(`${getFullApiHost()}/find_nexus?lat=${lat}&lng=${lng}`, {
    method: 'GET',
    headers: authHeader()
  }).then((promise) => {
...
function findAvailability (lat, lng, tenantId, loadType, carriage, availableHubIds, callback) {
  fetch(
    `${getFullApiHost()}/trucking_availability?` +
      `lat=${lat}&lng=${lng}&` +
      `tenant_id=${tenantId}&` +
      `load_type=${loadType}&` +
      `carriage=${carriage}&` +
...
function findTruckTypes (originNexusIds, destinationNexusIds, callback) {
  fetch(
    `${getFullApiHost()}/truck_type_availability?` +
